...
- In PR descriptions, issue comments, and any other external or project documentation, never cite local files, local paths, logs, or other artifacts that readers cannot access as evidence. Give evidence readers can inspect or reproduce: a self-contained reproduction command or code block, a branch or commit that contains the reproducible code, or the material facts transferred into the document itself. Local paths are acceptable only in direct reports to the user within the working session.
