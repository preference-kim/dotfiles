...
COOLDOWN="${VPN_WATCH_COOLDOWN:-60}"
KICKSTART_TIMEOUT="${VPN_WATCH_KICKSTART_TIMEOUT:-15}"
...
# Bound a command's runtime so a wedged launchctl call cannot hang the watchdog.
run_with_timeout() {
    local seconds="$1"
    shift
...
if run_with_timeout "$KICKSTART_TIMEOUT" launchctl kickstart -k "system/$LABEL"; then
    log "kickstart requested for $LABEL"
else
    log "kickstart failed or timed out for $LABEL"
    exit 1
fi
